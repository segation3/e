    	...
    	<table class="table shop_table_responsive shop_table cart woocommerce-cart-form__contents text-center" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr( $wishlist_id ) ?>" data-token="<?php echo esc_attr( $wishlist_token ) ?>">
            ...
            <tr>
    	        ...
    	        <th class="product-thumbnail">
    	            <span class="nobr"><?php echo __( 'Image', 'parlo' ); ?></span>
    	        </th>

                <th class="product-name">
                    <span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Product Name','parlo' ) ) ?></span>
                </th>
                ...
                <?php if( $show_price ) : ?>
                    ...
                    <th class="product-price">
                        <span class="nobr">
                            <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Unit Price','parlo' ) ) ?>
                        </span>
                    </th>
                ...
                <?php if( $show_stock_status ) : ?>
                    ...
                    <th class="product-stock-status">
                        <span class="nobr">
                            <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock Status','parlo' ) ) ?>
                        </span>
                    </th>
                ...
                <?php if( $show_last_column ) : ?>
                    ...
                    <th class="product-add-to-cart">
                        <span class="nobr"><?php echo __( 'Add To Cart', 'parlo' ); ?></span>
                    </th>
    	        ...
    	        <?php if( $is_user_owner ): ?>
    		        <th class="product-remove">
                        <span class="nobr"><?php echo __( 'Remove', 'parlo' ); ?></span>      
                    </th>
    	        <?php
    	            $column_count ++;
    	        endif;
            ...
            if( count( $wishlist_items ) > 0 ) :
                ...
                foreach( $wishlist_items as $item ) :
                    ...
                    if( $product && $product->exists() ) :
                        ...
                        <tr id="yith-wcwl-row-<?php echo esc_attr( $item['prod_id'] ) ?>" data-row-id="<?php echo esc_attr( $item['prod_id'] ) ?>">
                            ...
                            <?php if( $show_price ) : ?>
                                <td class="product-price">
                                    ...
                                    echo wp_kses_post( $base_product ) ? $product->get_price_html() : apply_filters( 'yith_free_text', __( 'Free!','parlo' ) ); 
                                    ?>
                                </td>
                            <?php endif ?>
                            ...
                            <?php if( $show_stock_status ) : ?>
                                <td class="product-stock-status">
                                    <?php echo wp_kses_post( $stock_status ) == 'out-of-stock' ? '<span class="wishlist-out-of-stock">' . __( 'Out of Stock','parlo' ) . '</span>' : '<span class="wishlist-in-stock">' . __( 'In Stock','parlo' ) . '</span>'; ?>
                                </td>
                            <?php endif ?>
                            ...
                            <td class="product-add-to-cart">
    	                        ...
    	                        if( $show_dateadded && isset( $item['dateadded'] ) ):
    								echo '<span class="dateadded">' . sprintf( __( 'Added on : %s','parlo' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
    	                        endif;
    	                        ?>
    	                        ...
    	                        <select class="change-wishlist selectBox">
    		                        <option value=""><?php _e( 'Move','parlo' ) ?></option>
    		                        <?php
    		                        ...
    		                        ?>
    			                        <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
    				                        ...
    				                        if( $wl['wishlist_privacy'] == 1 ){
    					                        $wl_privacy = __( 'Shared','parlo' );
    				                        }
    				                        elseif( $wl['wishlist_privacy'] == 2 ){
    					                        $wl_privacy = __( 'Private','parlo' );
    				                        }
    				                        else{
    					                        $wl_privacy = __( 'Public','parlo' );
    				                        }
    	                        ...
    	                        <?php if( $is_user_owner && $repeat_remove_button ): ?>
                                    <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php _e( 'Remove this product','parlo' ) ?>"><?php _e( 'Remove','parlo' ) ?></a>
                                <?php endif; ?>
                            </td>
    	                <?php endif; ?>
    	                ...
    	                <td class="product-remove">
    	                    <div>
    	                        <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove remove_from_wishlist" title="<?php _e( 'Remove this product','parlo' ) ?>"><i class="fa fa-times"></i></a>
    	                    </div>
    	                </td>
    	                <?php endif; ?>
                        </tr>
                ...
                <tr>
                    <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products were added to the wishlist','parlo' ) ) ?></td>
                </tr>
            <?php
            endif;
    	        ...
    	        <td colspan="<?php echo esc_attr( $column_count ) ?>">
    	            <?php if( $show_cb ) : ?>
    		            <div class="custom-add-to-cart-button-cotaniner">
    		                <a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_token ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', __( 'Add the selected products to the cart','parlo' ) ) ?></a>
    		            </div>
    	            <?php endif; ?>
    	            ...
    	            <?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
    		            <div class="ask-an-estimate-button-container">
    	                    ...
    	                    <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', __( 'Ask for an estimate','parlo' ) ) ?>
    	                </a>
    		            </div>
    	            <?php endif; ?>
	...
	<div id="ask_an_estimate_popup">
		<form action="<?php echo esc_url( $ask_estimate_url ); ?>" method="post" class="wishlist-ask-an-estimate-popup">
			<?php if( ! is_user_logged_in() ): ?>
				<label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', __( 'Your email','parlo' ) ) ?></label>
